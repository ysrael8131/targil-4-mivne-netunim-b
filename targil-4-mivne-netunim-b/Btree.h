...
public:
	...
	void deleteVal(T x);
	//homework
	void deleteTree() { deleteSubTree(root); root = NULL; }
	void printTree() { printSubTree(root); }
	void printTreeN(int n) { printSubTreeN(root, n); }
	BTree* search(T x) { return searchSubTree(root, x); }
private:
	//homework
	void deleteSubTree(BNode<T> *t);
	void printSubTree(BNode<T> *t);
	int printSubTreeN(BNode<T> *t, int n);
	BTree* searchSubTree(BNode<T>*t, T x);
...
template <typename T>
int BTree<T>::printSubTreeN(BNode<T> *t, int n)
{
	int n1;
	int total = 0;
	if (t == NULL)
	...
	{
		int i;
		for (i = 0; i < t->nsons && total < n; i++)
		{
			n1 = printSubTreeN(t->Son[i], n);
			total += n1;
